...
test('sauvegarde, météo, réglages, carte et missions',async({page})=>{
  ...
  await page.getByRole('button',{name:'Wax'}).click();
  await page.getByRole('button',{name:'Bordeaux'}).click();
  await expect(page.locator('#avatar-summary')).toHaveText('Ama · Awa · Tenue wax');
  await expect(page.getByRole('button',{name:'Wax'})).toHaveAttribute('aria-pressed','true');
  await expect(page.getByRole('button',{name:'Ville'})).toHaveAttribute('aria-pressed','false');
  await page.screenshot({path:'docs/audit/createur-personnage.png'});
  await page.getByRole('button',{name:'Commencer la balade'}).click();
  await expect.poll(()=>page.evaluate(()=>{const s=(window as any).__scenes.find((v:any)=>v.getObjectByName('joueur'));return s.getObjectByName('joueur').userData.apparenceJoueur;})).toEqual({couleur:'#9c4058',corps:'perso2.glb',style:'wax'});
  await expect.poll(()=>page.evaluate(()=>{const s=(window as any).__scenes.find((v:any)=>v.getObjectByName('joueur'));const j=s.getObjectByName('joueur');return {avatar:j.getObjectByName('corps-personnage')?.userData.avatar,tenue:!!j.getObjectByName('tenue-joueur-wax')};}),{timeout:20000}).toEqual({avatar:'perso2.glb',tenue:true});
  ...
  await expect(page.getByLabel('Prénom du personnage')).toHaveValue('Ama');await expect(page.getByRole('button',{name:/Awa/})).toHaveAttribute('aria-pressed','true');await expect(page.getByRole('button',{name:'Wax'})).toHaveAttribute('aria-pressed','true');
