...
export class Personnage {
  ...
  constructor(couleur:string,x:number,z:number,tenue:Tenue={}){
    ...
    const piece=(geo:T.BufferGeometry,c:string,x:number,y:number,z:number)=>{
      const m=new T.Mesh(geo,mat(c));m.position.set(x,y,z);m.castShadow=true;this.objet.add(m);return m;
    };
...
export class Joueur extends Personnage {
  ...
  deplacer(keys: Set<string>, yaw:number, dt:number, vitesse:number, paused:boolean, obstacles:Obstacle[], vehicule:boolean) {
    const dx=paused?0:Number(keys.has('d')||keys.has('arrowright'))-Number(keys.has('q')||keys.has('a')||keys.has('arrowleft'));
    const dz=paused?0:Number(keys.has('s')||keys.has('arrowdown'))-Number(keys.has('z')||keys.has('w')||keys.has('arrowup'));
    const moving=!!(dx||dz);
    if(moving){
      const len=Math.hypot(dx,dz);
      const vx=(dx*Math.cos(yaw)+dz*Math.sin(yaw))/len*vitesse*dt;
      const vz=(-dx*Math.sin(yaw)+dz*Math.cos(yaw))/len*vitesse*dt;
      const rayon=vehicule?.85:.3;
      const can=(x:number,z:number)=>x>-8+rayon&&x<8.2-rayon&&z>-407&&z<25&&!obstacles.some(o=>Math.abs(x-o.x)<o.w/2+rayon&&Math.abs(z-o.z)<o.d/2+rayon);
      const p=this.objet.position;
      if(can(p.x+vx,p.z))p.x+=vx;
      if(can(p.x,p.z+vz))p.z+=vz;
